...
defmodule BaseTest do
  ...
  test "start_link with init_ack/0" do
    ...
    assert { :ok, pid } = Base.start_link(BaseTest, fun)
  ...
  test "start_link with timeout" do
    ...
    assert { :error, :timeout } = Base.start_link(nil, BaseTest, fun,
  ...
  test "start_link with init_ignore" do
    ...
    assert :ignore === Base.start_link(BaseTest, fun)
  ...
  test "start_link with init_ack/1" do
    fun = fn(_parent, _debug) ->
      Base.init_ack(:extra_data)
      close()
    end
    assert { :ok, pid, :extra_data } = Base.start_link(BaseTest, fun)
  ...
  test "start_link with init_stop" do
    fun = fn(parent, debug) ->
      Base.init_stop(__MODULE__, parent, debug, nil, :init_error)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    assert { :error, :init_error } = Base.start_link(BaseTest, fun)
  ...
  test "start_link register local name" do
    ...
    assert { :ok, pid1 } = Base.start_link(name, BaseTest, fun)
    assert Base.whereis(name) === pid1
    trap = Process.flag(:trap_exit, true)
    assert { :error, { :already_started, ^pid1 } } = Base.start_link(name,
      BaseTest, fun)
  ...
  test "start with init_ack/0" do
    ...
    assert { :ok, pid } = Base.start(BaseTest, fun)
  ...
  test "start with timeout" do
    ...
    assert { :error, :timeout } = Base.start(nil, BaseTest, fun,
      [{ :timeout, 1 }]), "didn't timeout"
    assert TestIO.binread() === <<>>
  end
  ...
  test "start register local name" do
    ...
    name = :base_start
    assert { :ok, pid1 } = Base.start(name, BaseTest, fun)
    assert Base.whereis(name) === pid1
    assert { :error, { :already_started, ^pid1 } } = Base.start_link(name,
      BaseTest, fun)
    assert close(pid1) === :normal
  ...
  test "start with link" do
    ...
    assert { :ok, pid } = Base.start(nil, BaseTest, fun, [{ :spawn, [:link] }])
  ...
  test "spawn_link with init_ack/0" do
    ...
    pid = Base.spawn_link(BaseTest, fun)
    assert_receive { :parent, ^starter }, 200, "parent not starter"
    assert linked?(pid), "child not linked"
    assert close(pid) === :normal, "close failed"
  ...
  test "spawn_link with exception and stack init_stop" do
    ...
    fun = fn(parent, debug) ->
      Base.init_stop(__MODULE__, parent, debug, nil, { exception, stack })
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
  ...
  test "spawn_link with exception and nil stack init_stop" do
    exception = ArgumentError[message: "hello"]
    fun = fn(parent, debug) ->
      Base.init_stop(__MODULE__, parent, debug, nil, { exception, nil })
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
  ...
  test "spawn_link with init_stop and debug" do
    fun = fn(parent, debug) ->
      ...
      Base.init_stop(__MODULE__, parent, debug, nil, :init_error, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    ...
    log = "** Base.Debug #{inspect(pid)} event log:\n" <>
      "** Base.Debug #{inspect(pid)} :test_event\n\n"
  ...
  test "spawn_link with normal init_stop and debug" do
    fun = fn(parent, debug) ->
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.init_stop(__MODULE__, parent, debug, nil, :normal, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "spawn_link with shutdown init_stop and debug" do
    fun = fn(parent, debug) ->
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.init_stop(__MODULE__, parent, debug, nil, :shutdown, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "spawn_link with shutdown term init_stop and debug" do
    fun = fn(parent, debug) ->
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.init_stop(__MODULE__, parent, debug, nil, { :shutdown, nil }, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "spawn_link register local name" do
    ...
    assert pid1 = Base.spawn_link(name, BaseTest, fun)
    assert_receive { :registered, ^pid1 }, 200, "did not register"
    assert Base.whereis(name) === pid1
    assert pid2 = Base.spawn_link(name, BaseTest, fun)
  ...
  test "spawn register local name" do
    ...
    name = :base_spawn
    assert pid1 = Base.spawn(name, BaseTest, fun)
    assert_receive { :registered, ^pid1 }, 200, "did not register"
    assert Base.whereis(name) === pid1
    assert pid2 = Base.spawn(name, BaseTest, fun)
    ref = Process.monitor(pid2)
    assert_receive { :DOWN, ^ref, _, _, :normal }, 200,
      "already registered did not exit with reason :normal"
    refute_received { :registered, ^pid2 },
  ...
  test "spawn with link" do
    ...
    pid = Base.spawn(nil, BaseTest, fun, [{ :spawn, [:link] }])
    assert linked?(pid), "child not linked"
    assert :normal === close(pid), "close failed"
    assert TestIO.binread() === <<>>
  end
  ...
  test "stop" do
    fun = fn(parent, debug) ->
      Base.init_ack()
      Base.stop(__MODULE__, nil, parent, debug, :error)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
  ...
  test "stop with exception and stack" do
    ...
    fun = fn(parent, debug) ->
      Base.init_ack()
      Base.stop(__MODULE__, nil, parent, debug, { exception, stack })
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
  ...
  test "stop with exception and nil stack" do
    ...
    fun = fn(parent, debug) ->
      Base.init_ack()
      Base.stop(__MODULE__, nil, parent, debug, { exception, nil })
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
  ...
  test "stop and debug" do
    fun = fn(parent, debug) ->
      ...
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.stop(__MODULE__, nil, parent, debug, :error, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "stop with normal and debug" do
    fun = fn(parent, debug) ->
      ...
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.stop(__MODULE__, nil, parent, debug, :normal, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "stop with shutdown and debug" do
    fun = fn(parent, debug) ->
      ...
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.stop(__MODULE__, nil, parent, debug, :shutdown, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "stop with shutdown term and debug" do
    fun = fn(parent, debug) ->
      ...
      event = :test_event
      debug = Base.Debug.event(debug, event)
      Base.stop(__MODULE__, nil, parent, debug, { :shutdown, nil }, event)
      :timer.sleep(5000)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(nil, BaseTest, fun,
  ...
  test "uncaught exception" do
    ...
    Process.flag(:trap_exit, trap)
    report = "** Base #{inspect(pid)} is raising an exception\n" <>
  ...
  test "uncaught throw" do
    ...
    pid = Base.spawn_link(BaseTest, fun)
    assert_receive { :EXIT, ^pid, reason }, 200, "child did not exit"
    exception = Base.UncaughtThrowError[actual: :thrown]
    assert { ^exception, stack } = reason
    assert { __MODULE__, _, _, _ } = hd(stack),
      "stacktrace not from __MODULE__"
    Process.flag(:trap_exit, trap)
    report = "** Base #{inspect(pid)} is raising an exception\n" <>
      ...
      "   Pid: #{inspect(pid)}\n" <>
      "   Parent: #{inspect(self())}\n" <>
      "   (Base.UncaughtThrowError) uncaught throw: :thrown\n" <>
      Exception.format_stacktrace(stack) <>
      "\n"
    assert TestIO.binread() === report
  end
  ...
  test "uncaught exception after hibernate" do
    ...
    fun = fn(parent, debug) ->
      ...
      Base.hibernate(__MODULE__, :continue, fun2, parent, debug)
    end
    trap = Process.flag(:trap_exit, :true)
    pid = Base.spawn_link(BaseTest, fun)
    assert_receive { :EXIT, ^pid, reason }, 200, "child did not exit"
  ...
  test "call to pid" do
    ...
    pid = Base.spawn_link(BaseTest, fun)
    assert :hi === Base.call(pid, __MODULE__, :hello, 1000)
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "call to local name" do
    ...
    { :ok, pid } = Base.start_link(:call_local, BaseTest, fun)
    assert :hi === Base.call(:call_local, __MODULE__, :hello, 1000)
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "call to local name with local node" do
    ...
    { :ok, pid } = Base.start_link(:call_local2, BaseTest, fun)
    assert :hi === Base.call({ :call_local2, node() }, __MODULE__, :hello, 1000)
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "call to global name" do
    ...
    { :ok, pid } = Base.start_link(name, BaseTest, fun)
    assert :hi === Base.call(name, __MODULE__, :hello, 1000)
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "call to pid that is already dead" do
    ...
    assert_raise Base.CallError,
      ...
      fn() -> Base.call(pid, __MODULE__, :hello, 500) end
    assert TestIO.binread() === <<>>
  end
  ...
  test "call to local name that is not registered" do
    assert_raise Base.CallError,
      ...
      fn() -> Base.call(:call_bad_local, __MODULE__, :hello, 500) end
  end
  ...
  test "call to local name with local node that is not registered" do
    assert_raise Base.CallError,
      ...
      "no process associated with that name",
      fn() ->
        Base.call({ :call_bad_local, node() }, __MODULE__, :hello, 500)
      end
  end
  ...
  test "call to global name that is not registered" do
    assert_raise Base.CallError,
      ...
      "no process associated with that name",
      fn() ->
        Base.call({ :global, :call_bad_global }, __MODULE__, :hello, 500)
      end
  end
  ...
  test "call to via name that is not registered" do
    assert_raise Base.CallError,
      ...
      "no process associated with that name",
      fn() ->
        Base.call({ :via, :global, :call_bad_via }, __MODULE__, :hello, 500)
      end
  end
  ...
  test "call to local name on bad node" do
    assert_raise Base.CallError,
      ...
      fn() ->
        Base.call({ :call_bad_local, :node_does_not_exist }, __MODULE__, :hello,
          500)
      end
  end
  ...
  test "cast to pid" do
    ...
    pid = Base.spawn_link(BaseTest, fun)
    assert :ok === Base.cast(pid, __MODULE__, :hello)
  ...
  test "cast to local name" do
    ...
    { :ok, pid } = Base.start_link(:cast_local, BaseTest, fun)
    assert :ok === Base.cast(:cast_local, __MODULE__, :hello)
    assert_receive { :hi, ^pid }, 200, "cast was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "cast to local name with local node" do
    ...
    { :ok, pid } = Base.start_link(:cast_local2, BaseTest, fun)
    assert :ok === Base.cast({ :cast_local2, node() }, __MODULE__, :hello)
    assert_receive { :hi, ^pid }, 200, "cast was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "cast to global name" do
    ...
    { :ok, pid } = Base.start_link(name, BaseTest, fun)
    assert :ok === Base.cast(name, __MODULE__, :hello)
    assert_receive { :hi, ^pid }, 200, "cast was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "cast to local name that is not registered" do
    assert :ok === Base.cast(:cast_bad_local, __MODULE__, :hello)
  end

  test "cast to global name that is not registered" do
    assert :ok === Base.cast({ :global, :cast_bad_global }, __MODULE__, :hello)
  end

  test "cast to via name that is not registered" do
    assert :ok === Base.cast({ :via, :global, :cast_bad_via }, __MODULE__,
      :hello)
  end

  test "cast to local name on bad node" do
    assert :ok === Base.cast({ :cast_bad_local, :node_does_not_exist },
      __MODULE__, :hello)
  end
  ...
  test "send to pid" do
    ...
    pid = Base.spawn_link(BaseTest, fun)
    assert { __MODULE__, :hello } === Base.send(pid, {  __MODULE__, :hello })
  ...
  test "send to local name" do
    ...
    { :ok, pid } = Base.start_link(:send_local, BaseTest, fun)
    assert { __MODULE__, :hello } === Base.send(:send_local,
      { __MODULE__, :hello })
    assert_receive { :hi, ^pid }, 200, "message was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  ...
  test "send to local name with local node" do
    ...
    { :ok, pid } = Base.start_link(:send_local2, BaseTest, fun)
    assert { __MODULE__, :hello}  === Base.send({ :send_local2, node() },
      { __MODULE__, :hello })
    assert_receive { :hi, ^pid }, 200, "message was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  ...
  test "send to global name" do
    ...
    { :ok, pid } = Base.start_link(name, BaseTest, fun)
    assert { __MODULE__, :hello } === Base.send(name,{ __MODULE__, :hello })
    assert_receive { :hi, ^pid }, 200, "message was not received"
    close(pid)
    assert TestIO.binread() === <<>>
  end
  ...
  test "send to local name that is not registered" do
    assert_raise ArgumentError,
      ...
      fn() -> Base.send(:send_bad_local, { __MODULE__, :hello }) end
  end

  test "send to local name with local node that is not registered" do
    assert_raise ArgumentError,
      "no process associated with send_bad_local",
      fn() -> Base.send({ :send_bad_local, node() }, { __MODULE__, :hello }) end
  end
  ...
  test "send to global name that is not registered" do
    assert_raise ArgumentError,
      ...
      fn() ->
        Base.send({ :global, :send_bad_global }, { __MODULE__, :hello })
      end
  end
  ...
  test "send to via name that is not registered" do
    assert_raise ArgumentError,
      ...
      fn() ->
        Base.send({ :via, :global, :send_bad_via }, { __MODULE__, :hello })
      end
  end
